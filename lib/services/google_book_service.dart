  ...
  Future<List<GoogleBook>> searchBooks(String name) async {
    if (name != "") {
      ...
      for (int i = 0; i < listResponse.length; i++) {
        listResult.add(
          GoogleBook.fromApi(listResponse[i]),
        );
      }
...
class GoogleBook {
  ...
  String validadeAuthors(Map<String, dynamic> map) {
    return map["volumeInfo"]["authors"] == null
        ? "Autor desconhecido"
        : (map["volumeInfo"]["authors"] as List<dynamic>).map((e) => e).toString();
  }
